...
SELECT p.product_name, p.list_price, c.category_name FROM production.products p JOIN production.categories c ON p.category_id = c.category_id ORDER BY product_name; 
...
-- Write your query below:
SELECT c.first_name + ' ' + c.last_name as full_name, o.order_id, order_date FROM sales.customers c JOIN sales.orders o ON c.customer_id = o.customer_id ORDER BY order_date DESC;
...
-- Write your query below:
SELECT p.product_name, p.list_price, c.category_name, b.brand_name FROM production.products p JOIN production.brands b ON b.brand_id = p.brand_id JOIN production.categories c ON p.category_id = c.category_id ORDER BY brand_name, product_name;
...
-- Write your query below:
SELECT p.*, oi.order_id, oi.item_id FROM production.products p LEFT JOIN sales.order_items oi ON p.product_id = oi.product_id ORDER BY oi.order_id ASC;
...
-- Write your query below:
SELECT p.*, oi.order_id, oi.item_id FROM production.products p LEFT JOIN sales.order_items oi ON p.product_id = oi.product_id WHERE order_id IS NULL ORDER BY oi.order_id ASC;
...
-- Write your query below:
SELECT s.store_id, s.store_name, o.order_id, o.order_date FROM sales.stores s LEFT JOIN sales.orders o ON s.store_id = o.store_id;  
...
-- Write your query below:
SELECT s.first_name + ' ' + s.last_name as staff_name, m.first_name + ' ' + m.last_name AS manager_name FROM sales.staffs s INNER JOIN sales.staffs m ON s.manager_id = m.staff_id;
...
-- Write your query below:
SELECT s.store_name, b.brand_name FROM sales.stores AS s CROSS JOIN production.brands AS b;
--There are 27 rows in the result--
...
-- Write your query below:
SELECT c.first_name + ' ' + c.last_name as full_name, o.order_id, o.order_date, p.product_name, oi.list_price FROM sales.customers c JOIN sales.orders o ON c.customer_id = o.customer_id JOIN sales.order_items oi ON o.order_id = oi.order_id JOIN production.products p ON oi.product_id = p.product_id ORDER BY order_date, full_name;